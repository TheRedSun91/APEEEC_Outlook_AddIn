...
namespace APEEEC_Outlook_AddIn.src.Encryption
{
    class KeyManager
    {
        ...
        public void ImportKeyFromMessageAttachments(Outlook.Attachments attachments, String emailAddress) 
        {
            ...
            foreach (Outlook.Attachment attachment in attachments)
            {
                ...
                if (attachment.FileName == signedFileName)
                {
                    ...
                    //verify Signature
                    GpgInterfaceResult signatureResult = APEEEC_Broker.GetSingletonBroker().GetSignatureHandler().getSignature().VerifySignature(savedFile);
                    if (CallbackHandler.Callback(signatureResult, logger) == true)
                    {
        ...
        private void importKeyFromFile(string fileName)
        {
            GpgImportKey importingKey = new GpgImportKey(fileName);
            ...
            {
                logger.Warn("Key importing failed!");
                ImportKeyFailedForm importKeyFailedForm = new ImportKeyFailedForm();
                importKeyFailedForm.Show();
            }
        }
        ...
        internal void SendExchangeKeyRequest(Outlook.MailItem newMail, String senderEmail, Outlook.Recipient recipient)
        {
        ...
        internal void SendNegativeKeyExchangeResponse(Outlook.MailItem newMail, Outlook.AddressEntry recipient, string sender)
        {
            ...
            //add the body
            newMail.Body += "This message was generated by the APEEEC-Protocol and is used only to establish secure encrypted communication.";
            newMail.Body += "The sender " + sender + " denies your request for encrypted e-mail communication. If you wish to continue communicating with the sender please be aware that the communication will not be encrypted!";
            //send the mail
            newMail.Send();
            logger.Warn("Error-Handling required");
        }
        ...
        internal void SendPositiveKeyExchangeResponse(Outlook.MailItem newMail, Outlook.AddressEntry recipient, String sender)
        {
            ...
            if (CallbackHandler.Callback(result, logger) == true)
            {
                //retrieve the signed key from the saved location and add it to the mail as an attachment
                if (keyPath.Length > 0)
                {
                    newMail.Attachments.Add(
                        signedKeyPath,
            ...
            {
                //retrieve the unsigned key from the saved location and add it to the mail as an attachment
                if (keyPath.Length > 0)
                {
                    newMail.Attachments.Add(
                        keyPath,
